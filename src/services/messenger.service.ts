...
class MessengerService {
    ...
    public async handleMessage(sender_psid: string, received_message: any): Promise<void> {
        ...
        if (received_message) {
            ...
            if (received_message.includes('xsmb' || 'xổ sô' || 'xo so')) {
    ...
    public async handelQuickReply(sender_psid: string, received_message: any) {
        ...
        switch (payload) {
            ...
            case 'LIST_FONT_IMAGE':
                await this.sendListFontImage(sender_psid);
                break;
        }
    }
    public async sendTutorial(sender_psid: string) {
        ...
        await this.sendTextMessage(sender_psid, 'Nếu bạn muốn nhận hướng dẫn đầy đủ vui lòng gửi lại tin nhắn "HDSD"');
    }
    ...
    public async sendListFontImage(sender_psid: string) {
        await this.sendTextMessage(sender_psid, 'List font có hình ảnh');
    }
    ...
    public async sendCarouselMessage(sender_psid: string, elements: any) {
        // Create the payload for a basic text message
        let response = {
            attachment: {
                type: 'template',
